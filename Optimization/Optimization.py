...
dataTable = data[
            0:7146]  # rows 0:7146 (Excel calls them 2-7148; remember we skipped the blank first row in the read call) is the food data table
dataTable = dataTable.values.tolist()  # Convert dataframe to list

nutrientNames = list(
    data.columns.values)  # column headers (nutrient names are in columns 3-13; Excel calls them D-N)
...
cost = dict([(j[0], float(j[nutrientNames.index('Cholesterol')])) for j in
             dataTable])  # cost for each food

nutrients = []
for i in range(0, numNutrients):  # for loop running through each nutrient
    nutrients.append(dict([(j[0], float(j[i + 1])) for j in
                           dataTable]))  # amount of nutrient i in food j

# ------------ Create a new LP Problem ------------
#
# This problem is a minimization problem (find the *lowest* cost), so "LpMinimize" is the second parameter.

prob = LpProblem('Food optimization',
                 LpMinimize)  # 2 parameters: "name" and "sense"
...
for i in range(0, numNutrients):  # for loop running through each nutrient
    if (not np.isnan(minVal[0][i + 1])) and (not np.isnan(maxVal[0][
                                                              i + 1])):  # only write a constraint if upper and lower bounds exist
        print("adding constraint for " + nutrientNames[i + 1])
        prob += lpSum([nutrients[i][j] * foodVars[j] for j in foods]) >= \
                minVal[0][i + 1], 'min nutrient ' + nutrientNames[i + 1]
        prob += lpSum([nutrients[i][j] * foodVars[j] for j in foods]) <= \
                maxVal[0][i + 1], 'max nutrient ' + nutrientNames[i + 1]
...
for var in prob.variables():
    if var.varValue > 0:
        print(
            str(var.varValue) + " units of " + str(var).replace('Foods_', ''))
print()
